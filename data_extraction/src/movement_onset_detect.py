...
import math
from src.channel_maps import channel_label_map
...
def detectOnset(seqs_v_class_map, onsetAll, classes=1, channels=None, baselineEnd=32, threshV=1, threshdV=0.05, filt=17,
                plot=False):
    ...
    for i in range(0, nTrials):
        for c in range(0, nChan):
            seqs_v_class_map[classes][i][:, channels[c]] = seqs_v_class_map[classes][i][:, channels[c]] - np.mean(
                seqs_v_class_map[classes][i][0:baselineEnd, channels[c]])
...
def detectOnsetPCA(seqs_v_class_map, onsetAll, classes, channels=None, baselineEnd=32, threshV=1, threshdV=0.05,
                   filt=17, plot=False):
    nTrials = np.shape(seqs_v_class_map[classes])[0]
    nChan = np.size(channels)
...
def plotMRCP(ME_db_norm, ch=None):
    ...
    for c in range(0, nChan):
        for i in range(1, nClas + 1):
            for j in range(0, nTrial):
                std[c, :, i - 1] = np.square(
                    signal.lfilter(b, a, ME_db_norm[i][j][:, ch[c]]) - mean[c, :, i - 1]) + std[c, :, i - 1]
    ...
    for c in range(0, nChan):
        for i in range(0, nClas):
            axs[math.floor(c / 2), c % 2].plot(np.sqrt(std[c, :, i] / nTrial) / np.sqrt(nTrial),
                                               label=channel_label_map[c + 1])
        # axs[math.floor(c/2), c%2].set_ylim([-0.25,0.25])
        axs[math.floor(c / 2), c % 2].set_xlim([0, nPt])
        axs[math.floor(c / 2), c % 2].legend()
